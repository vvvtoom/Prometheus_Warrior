        ...
        case 37: // 왼쪽
            if (!char[0].mvFlag && char[0].charging == false && char[0].speed != 20 && !myCharDie) {
                ...
                if(!char[0].moving) {
                  ...
                  socket.emit('charBeh', data);
                }
            }
            if (char[0].charging == false && char[0].speed != 20){
              char[0].mvLeft = true;
              data = {
                job : "warrior",
                way : "left",
              ...
              socket.emit('charMoving', data);
            }
            break;
        case 38: // 위
            if (!char[0].mvFlag && char[0].charging == false && char[0].speed != 20 && !myCharDie) {
                char[0].state = 6;
                data = {job : "warrior", state : 6};
                socket.emit('charState', data);
                char[0].state_temp = 2;
                if(!char[0].moving) {
                  char[0].beh = 0;
            ...
            if (!char[0].FCool && !char[0].silence) {
                char[0].beh = 0;
                        switch (char[0].state) {
                            case 5:
                                char[0].state = 1;
                                data = {job : "warrior", state : 1};
                                socket.emit('charState', data);
                                break;
                            case 4:
                                char[0].state = 0;
                                data = {job : "warrior", state : 0};
                                socket.emit('charState', data);
                                break;
                            case 7:
                                char[0].state = 3;
                                data = {job : "warrior", state : 3};
                                socket.emit('charState', data);
                                break;
                            case 6:
                                char[0].state = 2;
                                data = {job : "warrior", state : 2};
                                socket.emit('charState', data);
                                break;
                          default:
                                char[0].state = char[0].state;
                                data = {job : "warrior", state : char[0].state};
                                socket.emit('charState', data);
                            break;
                        }
                        ...
                        rushcharge = setInterval(function() {
                            ...
                            if (char[0].charge == 100) {
                                ...
                                switch (char[0].state) {
                                    case 1:
                                        char[0].speed = 20;
                                        socket.emit('charSpeed', char[0].speed);
                                        char[0].mvLeft = true;
                                        char[0].rushing = true;
